...
class Dataset_importer:
    def __init__(self, client, project_names=None, project_ids=None, label_type='box', download=False, train_proba=0.8, val_proba=0.1):
        assert project_names is not None or project_ids is not None, "At least one of project_names or project_ids must be provided."
    ...
    def _set_project_ontology(self):
        if len(self.projects) == 0:
            raise ValueError("No projects found. Please check the project names or IDs.")
    ...
    def _set_path(self):
        ...
        if os.path.exists(os.path.join(os.getcwd(), 'datasets', self.name + '_' + str(count - 1))):
            self.prev_dataset_path = os.path.join(os.getcwd(), 'datasets', self.name + '_' + str(count - 1))
        else:
            self.prev_dataset_path = None
    ...
    def _set_available_classes(self):
        self.available_classes = [tool['name'] for tool in self.ontology.normalized['tools']]
    ...
    def _get_json(self):
        ...
        if self.download or self.prev_dataset_path is None or not os.path.exists(os.path.join(self.prev_dataset_path, "export_json.ndjson")):
            ...
            with open(os.path.join(self.prev_dataset_path, "export_json.ndjson"), "r", encoding="utf-8") as f:
                self.json_content = [json.loads(line) for line in f]

        # Save export_json as NDJSON file
        with open(os.path.join(self.path, "export_json.ndjson"), "w", encoding="utf-8") as f:
    ...
    def _separate_dataset(self):
        ...
        for data_row in self.json_content:
            ...
            if len(data_row['projects'][project_key]['labels']) > 0:
                for item in data_row['projects'][project_key]['labels'][0]['annotations']['objects']:
                    names.append(item['name'])
                    detected_element.append(self._manage_detected_item(item)) # TESTER LES DIFFÉRENTS TYPES DE LABELS
        ...
        self.train_set, _, self.val_set, val_classes = iterative_train_test_split(raw_dataset, present_classes, 1-self.train_proba)
        if self.val_proba + self.train_proba < 1:
            self.val_set, _, self.test_set, _ = iterative_train_test_split(self.val_set, val_classes, 1-self.val_proba/(1-self.train_proba))
        ...
        for k, item in enumerate(dataset):
            ...
            if labels["detection"] is not None:
                ...
                if label_type == 'box':
                    ...
                    save_boxes(img, labels["detection"], labels["names"], labels_path, self.available_classes)

                elif label_type == 'obb':
                    save_img_with_obb(img, labels["detection"], labels["names"], image_with_labels_path)
                    save_obb(img, labels["detection"], labels["names"], labels_path, self.available_classes)

                elif label_type == 'mask':
                    save_img_with_mask(img, labels["detection"], labels["names"], image_with_labels_path, self.client)
